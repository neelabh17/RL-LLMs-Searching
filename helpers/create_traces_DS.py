...
python helpers/create_traces_DS.py \
...
--num_generations 50 --push_to_hub
...
if __name__ == "__main__":
    ...
    for split in dataset_splits:
        dataset = load_dataset(final_ds, split=split) # use the train split only
        dataset = dataset.select(range(range_select[split]))
        ...
        # responses = llm.generate(prompts[:1], sampling_params, use_tqdm=True)
        responses = llm.generate(prompts, sampling_params, use_tqdm=True)

        # dump outputs to a file
        saving_ds_name = f"{args.dataset_name.split('_')[-1]}_out_of_the_box_num_gen_{args.num_generations}"
        ...
        if args.push_to_hub:
            ...
            final_dataset[split] = response_data
    ...
    if args.push_to_hub:
        ...
        dataset_final.push_to_hub(hf_dataset_name, token=hf_access_token)
